...
wxBoxSizer* MainFrame::create_side_tools()
{
    ...
    Slic3r::Hydra::HydraSyncManager::Instance().set_status_callback([this](Slic3r::Hydra::SyncState state, const std::string &msg) {
        wxGetApp().CallAfter([this, state, msg]() {
            ...
            if (state == Slic3r::Hydra::SyncState::InSync) m_hydra_status->SetLabel(_L("Hydra: in sync"));
            else if (state == Slic3r::Hydra::SyncState::PendingLocalChanges) m_hydra_status->SetLabel(_L("Hydra: local changes pending"));
            else m_hydra_status->SetLabel(_L("Hydra: sync failed"));
