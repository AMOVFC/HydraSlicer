...
{
    BOOST_LOG_TRIVIAL(debug) << "HydraGitSync: running: " << command;
...
std::string HydraGitSync::tokenized_remote_url() const
{
    ...
    if (m_remote_url.rfind(https_prefix, 0) != 0)
        return m_remote_url;
    return "https://x-access-token:" + m_access_token + "@" + m_remote_url.substr(https_prefix.size());
}
